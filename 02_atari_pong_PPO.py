...
def learning(num = 1):
    ...
    agent = AgentPPO(0.99, actions, Net(), 0.001, beta_entropy = 0.001, id=num, name='pong/')
    #agent.model.load_state_dict(torch.load('models/pong/pong_1_200_ppo.pt'))
    ...
    agent.learn(workers, 24, 100001)

def animation():
    actions = 4

    agent = AgentPPO(0.99, actions, Net(), lr = 2.5e-4, beta_entropy = 0.01, id=0, name='breakout/breakout_ppo.pt') #breakout/breakout_1_20000_a2c.pt'
    env = make_env('PongNoFrameskip-v4')
    agent.load_model()
