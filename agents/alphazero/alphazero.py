...
class Node:
    def __init__(self, probs, state, probs_mask, terminal, parent = None):
        self.state = state
        ...
        self.R = torch.zeros(length, dtype = torch.float32)
        self.T = terminal
...
class MCTS:
    ...
    def expansion(self, node, action, probs, state, probs_mask, terminal):
        new_node = Node(probs, state, probs_mask, terminal, node)
        node.children[action] = new_node
        return new_node
    def backup(self, node, value, actions):
        v = value * (1 - int(node.T))
        actions.reverse()

        for a in actions:
            node = node.parent
            v = node.R[a] - self.gamma * v
...
class AZAgent:
    def __init__(self, env, model, replay_buffer, actions, simulation_count = 800, gamma = 0.997, training_steps = 5000, c1 = 1.25, c2 = 19652.0, temperature_update_steps = 100000,
                 dirichlet_alpha = 0.3, exploration_fraction = 0.25, weight_decay = 0.0001, lr = 2e-2, arena_count = 25, self_play_steps = 100, batch_size = 512, name = 'az',
    ...
    def run_mcts(self, state, moves, env, model, training = False):
        ...
        for simulation in range(self.simulation_count):
            ...
            if a > -1:
                ...
                new_node = mcts.expansion(node, a, probs, new_state, new_moves, terminal)
    ...
    def play(self, model1, model2):
        ...
        return reward, logs, (- reward * player)
    ...
    def arena(self, model1, model2, iteration):
        ...
        for i in range(min_iteration, iteration):
            ...
            result, logs, reward = self.play(model1, model2)
            text += logs
            logs_win += str(game) + ' ' + str(reward)
            ...
            game += 1
            text += 'arena game ' + str(game) + '\n'
            result, logs, reward = self.play(model2, model1)
            text += logs
            logs_win += str(game) + ' ' + str(reward)
        ...
        return wins1 / float(wins1 + wins2 + 1e-10), text, logs_win
    ...
    def run(self):
        for iteration in range(1, self.run_iteration):
            ...
            print('arena')
            ratio, text, logs_win = self.arena(self.model, model2, iteration)
            print('arena ratio: ', ratio)

            f = open('logs/az/arena' + str(iteration) + '.txt', "w")
            f2 = open('logs/az/arena_win' + str(iteration) + '.txt', "w")
            f.write(text)
            f2.write(logs_win)
            f.close()
            f2.close()
