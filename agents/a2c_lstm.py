...
class AgentA2C:
    ...
    def choose_action(self, state, hx, cx):
        ...
        with torch.no_grad():
            logits, _, hx, cx  = self.model((state, hx, cx))
        probs = F.softmax(logits, dim=-1)
        probs = probs.cpu()
        action = probs.multinomial(num_samples=1).detach()
        return action[0].item(), hx, cx

    def learn(self, workers, max_steps, max_iteration, write=True, start_iteration=0, lstm_neurons = 256):
        len_workers = len(workers)
        observations, hx_vec, cx_vec = [], [], []
        for worker in workers:
            observations.append(torch.from_numpy(worker.reset()).float())
            hx_vec.append(torch.zeros(1, lstm_neurons))
            cx_vec.append(torch.zeros(1, lstm_neurons))
        observations = torch.stack(observations).to(self.device)
        hx_vec = torch.stack(hx_vec).to(self.device)
        cx_vec = torch.stack(cx_vec).to(self.device)
        ...
        for iteration in range(start_iteration, max_iteration):
            ...
            for step in range(max_steps):
                logits, values, hx_vec_old, cx_vec_old = self.model(observations, hx_vec, cx_vec)
                logits, values, hx_vec_old, cx_vec_old = logits.cpu(), values.cpu(), hx_vec_old.cpu(), cx_vec_old.cpu()
                ...
                for i in range(len_workers):
                    ...
                    hx, cx = hx_vec_old[i].detach(), cx_vec_old[i].detach()
                    if t == True:
                        non_terminals[i, 0] = 0
                        o = workers[i].reset()
                        hx = torch.zeros(1, lstm_neurons)
                        cx = torch.zeros(1, lstm_neurons)
